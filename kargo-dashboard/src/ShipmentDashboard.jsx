...
/**
 * KARGO OPERASYON MERKEZİ — Canlı Sipariş Takip Ekranı
 * ------------------------------------------------------
 * kargo-api'nin GET /shipment/list endpoint'inden besleniyor.
 * Backend hazır değilse otomatik olarak mock veriyle çalışır (aşağıdaki
 * USE_MOCK sabitini true yapman yeterli, ya da API'ye erişilemezse
 * kendiliğinden mock'a düşer).
 *
 * Projene eklemek için:
 *   1) Bu dosyayı src/ altına kopyala
 *   2) API_BASE_URL'i kendi kargo-api adresine göre ayarla
 *   3) <ShipmentDashboard /> olarak App.jsx içine koy
 */
...
const USE_MOCK_FALLBACK = true; // API'ye ulaşılamazsa mock veriyle devam et

// ---------------------------------------------------------------------
// Statü tanımları — worker'daki Türkçe karakterli ("BAŞARILI") ile
// enum'daki karaktersiz ("BASARILI") hallerini normalize ederek eşliyoruz.
// ---------------------------------------------------------------------
const normalize = (s = '') =>
  s
    .toString()
...
const getStatusMeta = (raw) => {
  ...
  return STATUS_MAP[key] || { label: raw || 'Bilinmiyor', color: '#5B6670', glow: 'rgba(91,102,112,0.25)', order: 9 };
};
...
// ---------------------------------------------------------------------
// Mock veri üretici (backend hazır değilken ekranı canlı göstermek için)
// ---------------------------------------------------------------------
function generateMock(count = 14) {
...
export default function ShipmentDashboard() {
  ...
  const fetchShipments = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/shipment/list`);
      ...
      if (USE_MOCK_FALLBACK) {
        // Hafifçe canlı hissettirmek için mock veriyi zaman zaman kıpırdat
        const jittered = mockRef.current.map((s) => ({ ...s }));
        setShipments(jittered);
        setIsMock(true);
  ...
  const filtered = useMemo(() => {
    return shipments
      .filter((s) => {
        if (statusFilter !== 'ALL' && normalize(s.status) !== statusFilter) return false;
        if (providerFilter !== 'ALL' && s.cargoProviderCode !== providerFilter) return false;
        if (search) {
          const q = search.toLowerCase();
          const hay = `${s.transactionId} ${s.consignmentNo} ${s.recipientName || ''}`.toLowerCase();
          if (!hay.includes(q)) return false;
        }
        return true;
      })
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  }, [shipments, statusFilter, providerFilter, search]);
  ...
  const providers = useMemo(
    () => Array.from(new Set(shipments.map((s) => s.cargoProviderCode))).filter(Boolean),
    [shipments],
  );
  ...
  return (
    <div style={styles.page}>
      ...
      <section style={styles.statStrip}>
        <StatCard label="Toplam Kayıt" value={totalCount} color="#E8EDF0" />
      ...
      <section style={styles.filterBar}>
        <input
          ...
          placeholder="TransactionID, konsinye no veya alıcı ara…"
          style={styles.searchInput}
        />
        <div style={styles.filterGroup}>
          <FilterPill active={providerFilter === 'ALL'} onClick={() => setProviderFilter('ALL')}>
            Tüm Sağlayıcılar
          </FilterPill>
          {providers.map((p) => (
            <FilterPill
        ...
        {loading ? (
          ...
          <div style={styles.emptyState}>
            ...
            <div style={styles.emptyText}>
              Filtreleri temizleyip tekrar dene, ya da yeni bir sevkiyat oluşturulmasını bekle.
            </div>
          </div>
        ) : (
          filtered.map((s) => (
      ...
      {error && !isMock && (
        <div style={styles.errorToast}>API hatası: {error}</div>
      )}
    </div>
  );
}
...
// ---------------------------------------------------------------------
// Stil sistemi — koyu "operasyon merkezi" teması, manifest/waybill motifi
// ---------------------------------------------------------------------
...
const styles = {
  page: {
    minHeight: '100vh',
    background: 'radial-gradient(ellipse at top, #141B1F 0%, #0C1013 60%)',
    color: '#E8EDF0',
    fontFamily: SANS,
